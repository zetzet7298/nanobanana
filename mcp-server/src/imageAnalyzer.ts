...
export class ImageAnalyzer {
  ...
  private getAnalysisPrompt(preset: EnhancementPreset): string {
    if (
...
`;
    ...
    try {
      if (
        ...
        if (cls.category && typeof cls.category === "string") {
          return {
            category: cls.category as ImageCategory,
            confidence:
              typeof cls.confidence === "number" ? cls.confidence : 0.8,
            subcategory:
              typeof cls.subcategory === "string" ? cls.subcategory : undefined,
            tags: Array.isArray(cls.tags) ? (cls.tags as string[]) : undefined,
          };
        }
      }
      ...
      if (categoryMatch) {
        ...
        const subcategoryMatch = analysisText.match(
          /"subcategory"\s*:\s*"([^"]+)"/i,
        );

        return {
          category: categoryMatch[1] as ImageCategory,
  ...
  async analyzeImage(
    ...
    try {
      ...
      try {
        ...
        if (jsonMatch) {
          parsedAnalysis = JSON.parse(jsonMatch[0]) as Record<string, unknown>;
        }
      } catch {
        console.error(
          "DEBUG - Could not parse JSON from analysis, using raw text",
        );
      }
      ...
      console.error(
        `DEBUG - Image classified as: ${classification?.category} (confidence: ${classification?.confidence})`,
      );
  ...
  private async callProxyForAnalysis(
    imageBase64: string,
    mimeType: string,
    prompt: string,
    ...
    try {
      ...
      const response = await fetch(
        `${this.localProxyBaseUrl}/v1beta/models/${this.modelName}:generateContent`,
        {
          method: "POST",
          headers: {
  ...
  async analyzeMultipleImages(
    ...
    console.error(
      `DEBUG - Analyzing ${images.length} images with concurrency ${concurrency}`,
    );

    for (let i = 0; i < images.length; i += concurrency) {
      const batch = images.slice(i, i + concurrency);
      const batchResults = await Promise.all(
        batch.map((img) => this.analyzeImage(img, presetName)),
      );
      results.push(...batchResults);

      console.error(
        `DEBUG - Analyzed batch ${Math.floor(i / concurrency) + 1}/${Math.ceil(images.length / concurrency)}`,
      );
    }
